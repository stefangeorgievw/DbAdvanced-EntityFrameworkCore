...
namespace MiniORM
{
	public abstract class DbContext
    {
            ...
            where TEntity : class, new()
        {
            ...
            var column = this.connection.FetchColumnNames(tableName).ToArray();

            if (dbSet.ChangeTracker.Added.Any())
            {
                this.connection.InsertEntities(dbSet.ChangeTracker.Added, tableName, column);
            }

            var modifiedEntities = dbSet.ChangeTracker.GetModifiedEntities(dbSet).ToArray();
            if (modifiedEntities.Any())
            {
                this.connection.UpdateEntities(modifiedEntities, tableName, column);
            }

            if (dbSet.ChangeTracker.Removed.Any())
            {
                this.connection.DeleteEntities(dbSet.ChangeTracker.Removed, tableName, column);
            }
        }
            ...
            foreach (var foreignKey in foreignKeys)
            {
                ...
                foreach (var entity in dbSet)
                {
                    ...
                    var navigationPropertyValue = ((IEnumerable<object>)navigationDbSet)
                        .First(currentNavigationPropery =>
                        navigationPrimaryKey.GetValue(currentNavigationPropery).Equals(foreignKeyValue));
            ...
            where TEntity: class
        {
            ...
            var column = GetEntityColumnName(table);

            var tableName = GetTableName(table);

            var fetchedRows = this.connection.FetchResultSet<TEntity>(tableName, column).ToArray();
        ...
        {
            var tableName = ((TableAttribute)Attribute.GetCustomAttribute(tableType, typeof(TableAttribute))).Name.SingleOrDefault().ToString();
